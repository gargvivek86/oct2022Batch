...
CHAINCODE_NAME="Carshowroom"
CHAINCODE_VERSION="1"
CHAINCODE_PATH="../chaincode/Carshowroom/lib/build/install/Carshowroom"
CHAINCODE_LANG="java"
CHAINCODE_LABEL="Carshowroom_1"
...
chaincodeAddCar() {
    ...
    --peerAddresses localhost:7051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG1\
    --peerAddresses localhost:9051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG2\
	-c '{"Args":["addNewCar", "3", "BMW","Vivek","20000"]}'
    echo "===================== Successfully Added Car Asset===================== "
}

chaincodequeryCarById() {
    echo "===================== Started Query Car Details Chaincode Function===================== "
    setEnvVarsForPeer0Org1
    peer chaincode invoke -o $ORDERER_ADDRESS\
    --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED\
    --cafile $ORDERER_CA -C ${CHANNEL_NAME} --name ${CHAINCODE_NAME}\
    --peerAddresses localhost:7051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG1\
    --peerAddresses localhost:9051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG2\
	-c '{"Args":["queryCarById", "3"]}'
    echo "===================== Successfully Invoked Query Car Details Chaincode Function===================== "
}
